...
namespace ConsoleClient
{
    ...
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // Цветовая схема (внешняя: светло-жёлтый + светло-голубой)
            // Цветовая схема (окно сообщений: )
            // Цветовая схема (окно сообщений: )
            ColorScheme colorMain = new ColorScheme();
            colorMain.Normal = new Terminal.Gui.Attribute(Color.BrightGreen, Color.Black);
            ColorScheme colorDark = new ColorScheme();
            colorDark.Normal = new Terminal.Gui.Attribute(Color.BrightGreen, Color.BrightBlue);
          ...
          //  menu.ColorScheme = colorMain;
            ...
            //winMessages.ColorScheme = colorMain;
        ...
        static void MessagesUpdate()
        {
            ...
            for (var i = 0 ; i < messages.Count; i++)
            {
                View msg = new View()
                {
                    ...
                    Height = 1,
                    Text = $"{messages[i].timestamp} [{messages[i].username}] > {messages[i].text}",
                };
                winMessages.Add(msg);
                offset++;
            }
